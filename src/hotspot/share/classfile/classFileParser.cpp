...
// 当parse_stream 解析完二进制字节码文件后，由类加载器为InstanceKlass分配所需内存，
// 然后由该方法结合解析出来的数据填充这片内存
// InstanceKlass实际上就是java.lang.Class在虚拟机级别的表示，记录了类有哪些字段，名字、类名、类型、解释器如何执行他的方法等
void ClassFileParser::fill_instance_klass(InstanceKlass* ik, bool changed_by_loadhook, TRAPS) {
  ...
  if ((_num_miranda_methods > 0) ||
      ...
      // 该标识位表示该类实现了某个接口，但是没有显式实现该接口的某些抽象抽象方法
      // 可能由其父类实现而隐式继承到了
  ...
  // 初始化itable，itable用于实现invoke_interface
...
void ClassFileParser::parse_stream(const ClassFileStream* const stream,
  ...
  stream->guarantee_more(3, CHECK); // length, first cp tag 长度，第一个常量池标记
