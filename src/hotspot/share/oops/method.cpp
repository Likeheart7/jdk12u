 ...
 * questions.
 * 对应java层面的Method
 * 包含了解释器入口(_from_interpreted_entry)、编译器入口(_from_compiled_entry)和用于统计方法性能的数据
...
// 方法链接
//各种入口的地址不会是一成不变的，当编译/解释模式切换时，入口地址也会相应切换，
// 如从解释器切换到编译器，编译完成后会设置新的_code、_from_compiled_entry和_from_interpreter_entry入口
