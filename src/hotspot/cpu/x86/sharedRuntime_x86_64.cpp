...
void SharedRuntime::gen_i2c_adapter(MacroAssembler *masm,
  ...
  // 将栈顶压入栈作为返回值，本地代码执行完毕后返回解释模式，就是使用这个地址
  __ push(rax);
  ...
  for (int i = 0; i < total_args_passed; i++) {
    ...
    //
    //
    // r_1和r_2都表示32位，组合起来，构成一个VMRegPair表示64位
    // 如果是64位则r_2无效，所以下面代码r_2>is_valid()相当于判断是否为64位
    VMReg r_1 = regs[i].first();
    VMReg r_2 = regs[i].second();
    if (!r_1->is_valid()) {
      assert(!r_2->is_valid(), "");
    ...
    } else { // 如果本地代码执行栈要求解释器栈参数放到XMM寄存器
      if (!r_2->is_valid()) {
  ...
  // rax
  __ mov(rax, rbx);
  __ jmp(r11); // r11保存了本地代码入口，所以跳到r11执行本地代码
}
